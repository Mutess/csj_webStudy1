...
public class GoodsModel {
	...
	public String goods_list(HttpServletRequest request, HttpServletResponse response) {
		...
		//DB연동
		GoodsDAO1 dao=GoodsDAO1.newInstance();
		List<GoodsVO> list=dao.goodListData(curpage, Integer.parseInt(type));
		//총페이지
	...
	@RequestMapping("goods/goods_datail.before.do")
	public String goods_detail_before(HttpServletRequest request, HttpServletResponse response) {
		...
		//  redirect => sendRedirect() => 기존의 제작된 메소드 (메소드를 호출)
		// => 반복 제거 => getConnection,
		// forward => 새로운 데이터를 전송
	...
	public String goods_detail(HttpServletRequest request, HttpServletResponse response) {
		...
		// DAO연결
		GoodsDAO1 dao=GoodsDAO1.newInstance();
		// 결과값을 request로 묶어서 => good_detail.jsp
