...
public class FreeboaredDAO1 {
	private Connection conn;
	private PreparedStatement ps;
	private CreateDataBase db=new CreateDataBase();
	private static FreeboaredDAO1 dao;
	
	public static FreeboaredDAO1 newInstance() {
		if(dao==null)
			dao=new FreeboaredDAO1();
		return dao;
	}
	// 기능
	// 1. 목록 출력 => 인라인뷰 => 페이징
	public List<FreeBoardVO> freeboardListData(int page){
		...
		try {
			...
			String sql="SELECT no, subject, name, TO_CHAR(regdate,'yyyy-mm-dd'), hit, rownum "
	...
	// 2. Top7 => rownum (hit가 많은 위에서부터 7) => Top-N
	// 3. 글쓰기
