		...
		내장 객체 (일반 JSP ==> 자바/HTML 분리)
			...
			= requestScope =>
			  ...
			  request.getAttribute("key")
			  	 |
			  ${requestScope.key} => {key}=> 오라클에서 얻은 값을 추가, requestScope가 생략이 가능해서 key만 작성해도 됨
			= sessionScope
			  session에 저장된 값을 읽기
			  ${sessionScope.key} => ${key} => 우선순위:request=>session
			  
			= param : ${param.key} => request.getParameter("key")
			= paramValues : ${paramValues.key} => reqeust.getParameterValues()
...
<%
	...
	names.add("박문수");
	names.add("강감찬");
	// 이 상태에선 EL은 사용할 수가 없음 (EL ${}=> request, session에 저장을 해야 사용이 가능)
