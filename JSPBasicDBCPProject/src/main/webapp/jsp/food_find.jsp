<%@page import="com.sist.dao.FoodDAO12"%>
...
<%-- 
	...
	3. 쉽게 서버가 다운되지 않는다
	4. 라이브러기가 만들어져 있다 (commons-dbcp, commons-pool)
	...
	==> 280page
		목적 : 웹 사용자는 응답 시간 (빠르게)	
		 ...
		 DBCP
		 ----
		 방법 => 1. Connenction 객체 생성 (maxActive : 최대, maxIdle : 풀)
		 		2. POOL영역에 저장
		 		3. 사용자가 요청시에 Connection의 주소를 얻어온다.
		 		4. 사용자 요청에 따라 수행
		 		5. 반드시 POOL안에 Connection객체를 반환
		 		----------------------------------
		 		1. server.xml => Connection객체를 생성
		 		2. 코딩 방법
		 		   ------
 ...
 <%
 	...
 	FoodDAO12 dao=FoodDAO12.newInstance();
...
$(function(){
	$("#finfBtn").click(function(){
		let addr=$('#addr').val();
		if(addr.trim()==="") {
			$('#addr').focus();
			return;
		}
		$('#frm').submit(); // 유효성 검사 (강제입력)
	})
})
</script>
<body>
		...
		<div class="row">
			<div class="text-center">
				<ul class="pagination">
					...
					%>
					<li <%=i==curpage?"class=active":"" %>><a href="food_find.jsp?page=<%=i%>&addr=<%=fd%>"><%=i %></a></li> <%-- 페이지 넘기고 주소 출력 --%>
					<%
						}
					%>
					<%
