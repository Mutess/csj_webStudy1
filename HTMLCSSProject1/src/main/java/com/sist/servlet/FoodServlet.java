...
/*								  	 SQL문장 전송
		브라우저  <===========> 자바 <===============> 오라클
				 HTML로 변환	  |     실행된 데이터 읽기
				 			...
				 			=> 수정시마다 컴파일을 한다
				 			=>-------------------- 보완 : JSP
				 			=> servlet,jsp => 호출하는 것이 아니라 Tomcat에 의해서 호출
				 			=> URL주소를 이용해서 호출
				 => new FoodServlet() => 톰캣
				 	1) init() : 변수 초기화, 필요한 값을 읽는 경우
				 				자동 로그인, 보안...
				 	------------------------------------
				 	2) servlet() : 사용자가 요청한 HTML을 만드는 위치
				 		= doGet() => GET
				 		= doPost() => POST
				 		= 브라우저로 HTML을 전송하는 위치
				 	------------------------------------ JSP(메소드 영역) 
			...
			a.jsp => a_jsp.java
					 ...
					 %>
					 HTML
					 => servlet (Spring 라이브러리가 servlet으로 제작됨)
...
public class FoodServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		FoodDAO2 dao = FoodDAO2.newInsatance();
		List<FoodVO> list = dao.foodAllData();
		PrintWriter out = response.getWriter(); 
		// PrintWriter => 브라우저에 출력하는 내용들 => HTML
		out.println("<html>");
		out.println("<head>");
		...
		out.println("let temp=$('#user-table > tbody > tr > td:nth-child(5n+2):contains(\"'+k+'\"')"); //nth-child(5n+2) 이 영역에서만 검색을 하겠다 (column 5개)
		...
		out.println("<thead>"); //한번만 실행할때 thead를 사용
		out.println("<tr>");
		out.println("<th></th>");
		...
		out.println("</tr>");
		out.println("</thead>");
		out.println("<tbody>"); // 실제 값 출력 // Vue, React(View)에서는 table출력이 안됨
		for (FoodVO vo:list) {
			out.println("<tr>");
