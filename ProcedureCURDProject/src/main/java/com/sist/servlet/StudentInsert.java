...
public class StudentInsert extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<form method=post action=StudentInsert>"); //doPost호출시에 사용
		// post => doPost
		// 나머지는 doGet
		/*
			<form method=get> doGet()
			<form method=aaa> doGet() => 오류가 나도 Get방식
			<form> doGet()
		...
		out.println("<tr>");
		out.println("<td colspan=2 class=text-cen>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 한글 => 디코딩
		// 사용자가 보내준 값을 받는다.
		// <input type=number name=name> name속성을 설정하지 않으면 null값을 출력
		// servlet에서만 적용되는 것이 아닌 jsp/spring에서도 포함
		//								 ------ servlet으로 만들어져서 그럼
