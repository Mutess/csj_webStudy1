	...
	3) 내장 객체 : 미리 객체를 생성해서 필요한 위치에서 사용이 가능
		request : HttpServletRequest
					...
					= 요청정보
						...
						* setCharacterEncoding() : 디코딩
						=> POST에서만 사용
						=> GET은 자동화 처리 => windows10 이상
						   ------------- server.xml(한글처리,포트)
						   8080dms 프록시서버
					= 추가정보 (MVC,MV) => 오라클에서 받은 값을 추가 => JSP
						*setAttribute() : 기존에 있는 request값에 출력에 필요한 데이터를 추가해서 사용
		...
		response : 응답정보, 화면 이동 정보 => HttpServletResponse
				  ...
				  Header 정보 : 실제 데이터 전송 전에 처리
				  	setHeader() => 한글 변환 / 다운로드시에 파일명, 크기 => 다운로드창을 띄워줌
		out : JspWriter (출력중인 버퍼 관리)
			  출력 (메모리)
			  println() <%= %>
...
<%
	      ...
	      for (EmpVO vo:list) { //14줄
		      ...
		      out.print("<td class=text-center><a href=\"MainServlet?mode=2&empno="+vo.getEmpno()+"\">"+vo.getEname()+"</a></td>"); //넘기는 값이 ID나 번호를 넘김
