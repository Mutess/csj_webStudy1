...
/*
	...
	연결 (화면, 구현) : 서블릿
		=> Controller
	------------------------------------------------
	model 1 (순수하게 JSP) => 장점 : 분석하기 편리. 단점 : 확장성이 없고, 추가, 재사용이 어렵다.
	model 2 (JSP/Java 분리) => 단점 : 복잡함. 장점 : 여러명이 동시에 개발, 
											   추가, 확장성, 재사용 용이 => 유지보수
	...
	MVC의 동작구조
	-----------
	* Controller를 찾는 방법 : .do로 주면 Controller를 찾아감 (URL주소 설정)
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			for(String cls:clsList) {
				Class clsName=Class.forName(cls); //클래스 정보를 읽음 Class.forName
