...
public class BoardUpdateServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//form만 post방식으로 데이터를 전송
		//post방식으로 보내면 => method=post => doPost()로 보내서 전송
		//입력한 데이터를 한번에 => action에 등록된 클래스로 전송
		...
		out.println("<td width=85%><input type=text name=subject size=50 required value=\""+vo.getSubject()+"\"></td>"); // ""가 들어야되기 때문에 \""+vo+"\">를 사용
		out.println("</tr>");
		out.println("<tr>");
	 ...
	 */
	//http://localhost:8080/HTMLProject/BoardInsertServlet?name=%ED%99%8D%EA%B8%B8%EB%8F%99&subject=%EC%B2%98%EC%9D%8C+%EB%A7%8C%EB%93%9C%EB%8A%94+%EA%B2%8C%EC%8B%9C%ED%8C%90&content=&pwd=134
