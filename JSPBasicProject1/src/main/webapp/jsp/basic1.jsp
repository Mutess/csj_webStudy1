<%-- 
	...
	3. Servlet VS JSP
		Servlet
		JSP
		----------------------- 웹서비스 가능 (사용자 요청 => HTML로 변환해서 화면 출력)
		=> Servlet은 자바파일 => 수정시마다 컴파일을 해야한다.
			1. 변경시마다 컴파일
			2. HTML이 자바안에서 코딩 => 문자열
			3. CSS를 구사하기 어렵다 ... 개발자 (HTML,CSS,Java,Oracle을 다 해야함)
		=> 단점을 보안 : JSP (HTML을 쉽게 만든다.)
		=> 장점
			서블릿보다 쉽고 작성하기 빠르다
	...
	5. JSP 문법
		...
		***= MVC
		...
		-------------
		 ...
		 자바언어로 사용된다
		 자바 / HTML로 분리되어 있다
		 JSP는 항상 => 서블릿으로 변경후에 사용
		 			 ----------------- 톰캣에 의해 변경
		 JSP는 화면에 출력할 내용의 메소드인데 들어가는 내용을 코딩함
		 --- 클래스가 아니고 메소드의 영역
		 _jspService() {
	...
	26page
	 ...
	 => Servlet (28page)
	 	JSP와 연결해서 처리
	 	JSP : 정적페이지 => 코딩하기가 편리함 (자바/HTML) 구분
	 ...
	 ------------------
	  ...
	  3. servlet
	  	 ...
	  	 init()
	  	 	|
	  	 service() ===> 브라우저에 요청한 내용애 대한 처리 = 응답한 HTML작성
	  	 	= doGet()
	  	 	= doPost()
	  	 	|
